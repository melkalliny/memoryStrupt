function [corr_ConsistencyVsBehavior] = consolidation_calculateSimilarity_LowAndHigh(subjectNames, subjectsToRun, stringEvent, windowOptions)
...
windowLength = [100 200 500 1000 2000];
windowShift = [10 20 50 100 200];

i_full=[111;51;21];

totalWindows = i_full(windowOptions);
...
for n = subjectsToRun  % 2,4,6 = identical-session subjects, with performance marked in events.mat
    ...
    load(sprintf('/Volumes/FRNU_SVN/consolidation_SVN/consolidationProject/intermediates/processedPower_LowAndHigh_monopolar_window%d/%s/allPowData.mat',windowOptions,subjectNames{n}))
...
%                 if windowType == 2
%                     
%                     windowsToStartAt = 1:100:5i_full(windowType);
%                     %windowsToStartAt = windowsToStartAt(1:4:end);
%                     new_power = NaN(size(power{4,i},1),size(power{4,i},2),10*6,size(power{4,i},4));
%                     for shift=1:10
%                     for j=1:length(windowsToStartAt)
%                         if j==length(windowsToStartAt)
%                         new_power(:,:,shift*j,:) = nanmean(power{4,i}(:,:,[windowsToStartAt(j)+(shift-1):10:end],:),3);    
%                         else
%                         new_power(:,:,shift*j,:) = nanmean(power{4,i}(:,:,[windowsToStartAt(j)+(shift-1):10:windowsToStartAt(j)+(shift-1)+100],:),3);
%                         end
%                     end
%                     end
%                     power{4,i} = new_power;
...
%                 elseif windowType==3
%                     
%                     windowsToStartAt = 1:50:5i_full(windowType);
%                     %windowsToStartAt = windowsToStartAt(1:4:end);
%                     new_power = NaN(size(power{4,i},1),size(power{4,i},2),10*6*2,size(power{4,i},4));
%                     for shift=1:10
%                     for j=1:length(windowsToStartAt)
%                         if j==length(windowsToStartAt)
%                         new_power(:,:,shift*j,:) = nanmean(power{4,i}(:,:,[windowsToStartAt(j)+(shift-1):10:end],:),3);    
%                         else
%                         new_power(:,:,shift*j,:) = nanmean(power{4,i}(:,:,[windowsToStartAt(j)+(shift-1):10:windowsToStartAt(j)+(shift-1)+50],:),3);
%                         end
%                     end
%                     end
%                     power{4,i} = new_power;                    
...
%                 end
           ...
           % wordStrings(find(isempty(wordStrings))) = [];
            ...
            elseif strcmpi(pairwiseOrEnds,'pairwise')
                ...
                consistencyByTime = NaN(156,totalWindows,2,size(power,2)-1); % 3 window sizes. 100, 250, and 500
                consistencyByTime_patterns = cell(156,totalWindows,2,size(doTheseSess,2));
...
%                             
            ...
            corr_RT= NaN(2,totalWindows,3);
            corr_Content = NaN(2,totalWindows,3);
            corr_Context = NaN(2,totalWindows,3);
            corr_ContentCorrect = NaN(2,totalWindows,3);
            corr_ContentIncorrect = NaN(2,totalWindows,3);
            
            for freqs=1:2
                
                
                for window=1:totalWindows
...
%                     idealized_content_matrix = (idealized_content_matrix-tempmean) / tempstd;
                    % and the session level contrast weightsnih
