...
for n = [4] % 2,4,6 = identical-session subjects, with performance marked in events.mat
    ...
    load(sprintf('/Volumes/56PUB/UTAH_RAFI/consolidationProject/intermediates/processedRipples/%s/allRipples.mat',subjectNames{n}))
    ...
    for studyType=studyTypeToRun
        ...
        for location=locationToRun
            ...
            for i=1:length(sessions)
                ...
                temp_data = squeeze(nanmean(temp_data,2)); %average across elecs
...
%             end
            ...
            for category=[1,2,3]
               ...
               % databar = bar(session,[nanmean(data)],0.5,'FaceColor','none','EdgeColor','k','LineWidth',2);
                ...
                if session>1
                    try
                    aggregate = cat(1,power{category,session-1},data);
                    catch
                    aggregate = cat(1,power{category,session-1}',data');    
                    end
                    if size(aggregate,1)==2
                        aggregate = cat(2,aggregate(1,:),aggregate(2,:)); 
                    end
            ...
            box off
            xlabel('session')
            ylabel('# of ripples during encoding')
            %ylim([0 25]); 
