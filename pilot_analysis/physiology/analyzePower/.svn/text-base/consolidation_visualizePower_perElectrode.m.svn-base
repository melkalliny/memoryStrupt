...
for n = [2,4,6] % 2,4,6 = identical-session subjects, with performance marked in events.mat
    
    load(sprintf('/Volumes/56D/UTAH_RAFI/consolidationProject/intermediates/processedPower/%s/allPowData.mat',subjectNames{n}))
    ...
    for studyType=studyTypeToRun
        ...
        for location=locationToRun
            ...
            for i=1:length(sessions)
                ...
                for word=1:length(pull_evs)
                    ...
                    if i==1
                        ...
                        elseif (category_word(word,i) == 1) && (category_word(word,i-1) == 1)
                            session_words(word) = 1; % consolidated category
                        elseif (category_word(word,i) == 0) && (category_word(word,i-1) == 0)
                            session_words(word) = -1; % nonconsolidated category                            
                        end
                ...
                power{4,i} = temp_data;
                power_raw{1,i} = squeeze((processed_power{3,location}(pull_evs_index,:))); %pre-stim data                
                power_raw{2,i} = squeeze((processed_power{4,location}(pull_evs_index,:))); %post-stim data
...
%             end
            ...
            for category=[1,2,3]
                ...
                data = power{category,session}(:,elec);
                hold on
               ...
               % databar = bar(session,[nanmean(data)],0.5,'FaceColor','none','EdgeColor','k','LineWidth',2);
                ...
                if session>1
                    aggregate = cat(2,power{category,session-1}(:,elec)',data');
                    aggregateSess = cat(2,repmat(session-1,1,size(power{category,session-1},1)),repmat(session,1,length(data)));
            ...
            title(sprintf('%s, %s, %s, elec %d',subjectNames{n},locationStrings{1,location},evTypeStrings{1,studyType},elec))
            ...
            for session=1:length(sessions)
                hold on
                x_vals = linspace(session,session+1,size(power_raw{1,session},1));
                
                positive = []; negative = [];
                
                for point=1:length(power_raw{1,session}(:,elec))
                    hold on
                    if (power_raw{1,session}(point)-power_raw{2,session}(point,elec)) > 0
                        line([x_vals(point) x_vals(point)],[power_raw{1,session}(point,elec) power_raw{2,session}(point,elec)],'Color','r')
                        positive = cat(2,positive,power_raw{3,session}(point));
                    else
                        line([x_vals(point) x_vals(point)],[power_raw{1,session}(point,elec) power_raw{2,session}(point,elec)],'Color','k')
           ...
           set(gca,'FontSize',18)
            ...
            for session=1:length(sessions)
                aggregate = []; aggregateSess = [];
                data = power{4,session}(:,elec); %4=all data
                hold on
