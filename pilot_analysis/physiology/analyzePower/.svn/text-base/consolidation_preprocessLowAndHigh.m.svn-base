function [out] = consolidation_preprocessLowAndHigh_allTime(windowNum)
...
for n = [1,3]
    ...
    load(sprintf('/Volumes/FRNU_SVN/consolidation_SVN/consolidationProject/intermediates/power_LowAndHigh_monopolar//%s/powDir/allPowData.mat',subjectNames{n}))
    ...
    load(sprintf('/Volumes/FRNU_SVN/consolidation_SVN/consolidationProject/intermediates/power_LowAndHigh_monopolar/%s/powDir/events.mat',subjectNames{n}))
    ...
    for sess = 1:length(uniqueSessions)
        ...
        for elec=1:size(powDataAll,2)
            for freq=1:size(powDataAll,4)
                meanDistrib = nanmean(nanmean((powDataAll(eventsBaselineInSession,elec,10:20,freq))));
                stdDistrib = nanmean(nanstd(powDataAll(eventsBaselineInSession,elec,10:20,freq)));
                for ev=1:length(eventsInSession)
                    powDataAllZscored(eventsInSession(ev),elec,1:119,freq) = ((powDataAll(eventsInSession(ev),elec,1:119,freq)) - meanDistrib) / stdDistrib;
                end
            end
        end
    end
   ...
   if ~exist(sprintf('/Volumes/FRNU_SVN/consolidation_SVN/consolidationProject/intermediates/processedPower_LowAndHigh_monopolar/%s/',subjectNames{n})) %#ok<EXIST>
       mkdir(sprintf('/Volumes/FRNU_SVN/consolidation_SVN/consolidationProject/intermediates/processedPower_LowAndHigh_monopolar/%s/',subjectNames{n})); 
   end
   
   save(sprintf('/Volumes/FRNU_SVN/consolidation_SVN/consolidationProject/intermediates/processedPower_LowAndHigh_monopolar//%s/allPowData.mat',subjectNames{n}),'processed_power','evsOfInterest','-v7.3')
